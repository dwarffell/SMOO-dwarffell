...
private:
    ...
    float cooldownRate = 0.25f; // Rate the power is recharged
...
public:
    ...
    PlayerWallActionHistory* mPlayerWallHistory;
    bool isPInWater = false; // Bool holding if the player is in water

    // Game Time
    GameTime curGameModeTime; // Most recent time from the Hide and Seek gamemode
    TimeWarpCharge* timeWarpLayout;
    ...
    sead::Color4u8 calcColorFrameU8(float colorFrame, int dotIndex); // Calculates the sead::Color4u8 of the colorFrame
