...
bool SocketClient::send(Packet* packet) {
    ...
    int fd = -1;
    if (
        (packet->mType != PLAYERINF && packet->mType != HACKCAPINF && packet->mType != HOLEPUNCH)
...
bool SocketClient::recvTcp() {
    ...
    if (header->mType != PLAYERINF && header->mType != HACKCAPINF) {
        Logger::log("Received packet (from %02X%02X):", header->mUserID.data[0], header->mUserID.data[1]);
        Logger::disableName();
    ...
    while (valread < fullSize) {
        int result = nn::socket::Recv(fd, packetBuf + valread, fullSize - valread, this->sock_flags);
...
void SocketClient::printPacket(Packet* packet) {
    ...
    case PacketType::PLAYERINF:
        Logger::log(
            ...
            ((PlayerInf*)packet)->playerPos.y,
            ((PlayerInf*)packet)->playerPos.z
        );
        Logger::log(
            "Rot X: %f Rot Y: %f Rot Z: %f\nRot W: %f\n",
            ((PlayerInf*)packet)->playerRot.x,
            ((PlayerInf*)packet)->playerRot.y,
            ((PlayerInf*)packet)->playerRot.z,
