...
void GameModeTimer::updateTimer()
{
    ...
    container.curGameModeTime = mTime;
    ...
    if (mIsEnabled) {
        ...
        } else if (mTime.mMilliseconds < 0 && (mTime.mSeconds > 0 || mTime.mMinutes > 0 || mTime.mHours > 0)) {
            mTime.mMilliseconds++;
            mTime.mSeconds--;
            if (mTime.mSeconds < 0) {
                mTime.mSeconds = 59;
    ...
    if (container.isRewind())
        setTime(container.getTimeAtFrame());
}
